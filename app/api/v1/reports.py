...
@router.get("/applications/{application_id}/feedback", response_model=CandidateFeedbackOut)
async def get_interview_feedback_for_candidate(
        application_id: int,
        db: AsyncSession = Depends(get_db),
        current_user: User = Depends(get_current_candidate_user)
):
    app_repo = ApplicationRepository(db)
    application = await app_repo.get_application_with_report(application_id)
...
async def get_interview_transcript(
    ...
    transcript_entries = await interview_repo.get_transcript_for_session(session_id)
