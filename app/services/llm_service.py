...
def evaluate_answer_depth(question: str, answer: str) -> dict:
    system_prompt = (
        "Ты — AI-супервайзер. Оценивай ответ по сути, игнорируя речевые ошибки, повторы и слова-паразиты. "
        "Это живая речь, а не письменный экзамен. Ответ — ТОЛЬКО JSON."
    )
    user_prompt = f"""
...
def get_interview_response(history: list[dict], vacancy_details: dict, last_answer_analysis: dict, behavior_flags: dict) -> str:
    system_prompt = (
        "Ты — харизматичный HR-супервайзер, управляющий AI-аватаром Алексом. "
        "Твоя задача — сгенерировать СЛЕДУЮЩУЮ реплику для Алекса. "
        "ВАЖНО: Не начинай каждую реплику с имени кандидата. Используй его изредка. "
        "Говори естественно и по-деловому, но с эмпатией. Ответ — ТОЛЬКО текст реплики."
    )
    vacancy_context = json.dumps(vacancy_details, ensure_ascii=False)
    user_prompt = f"""
    Проанализируй ситуацию в интервью и реши, что Алексу сказать дальше.
    **КОНТЕКСТ ВАКАНСИИ:** {vacancy_context}
    **АНАЛИЗ ПОСЛЕДНЕГО ОТВЕТА:**
    - Оценка глубины: {last_answer_analysis.get('depth_score', 3)}/5
    - Вывод: {last_answer_analysis.get('summary', 'N/A')}
    - Флаги поведения: {json.dumps(behavior_flags, ensure_ascii=False)}
    **ИСТОРИЯ ДИАЛОГА (последние реплики):** {json.dumps(history[-4:], ensure_ascii=False)}

    **ТВОЯ ЗАДАЧА (выполни по шагам):**
    1.  **Оцени ситуацию:** `is_off_topic`? `depth_score` <= 2 (слабый ответ)? `depth_score` >= 4 (сильный ответ)?
    2.  **Выбери тактику (ОДНУ):**
        - **A (Копать глубже):** Если ответ сильный, задай уточняющий вопрос по этой же теме.
        - **B (Переформулировать):** Если ответ очень слабый, но по теме, возможно, кандидат не понял. Переформулируй вопрос проще.
        - **C (Вернуть к теме):** Если `is_off_topic`=true, вежливо верни к последнему вопросу.
        - **D (Сменить тему):** Во всех остальных случаях, плавно перейди к следующему нераскрытому критерию.
        - **E (Подбодрить):** Если кандидат явно нервничает или извиняется, скажи что-то ободряющее перед вопросом.
    3.  **Сгенерируй реплику:** На основе тактики напиши ОДНУ реплику для Алекса.
    Твой ответ:
    """
    response_text = _call_yandex_gpt(system_prompt, user_prompt, temperature=0.7)
    if not response_text or not response_text.strip():
        return "Хорошо, спасибо. Давайте перейдем к следующему вопросу."
...
def generate_greeting_phrase(candidate_name: str, vacancy_title: str, first_question: str | None) -> str:
    system_prompt = (
        "Ты — AI-аватар Алекс. Твоя задача — вежливо и креативно поприветствовать кандидата и задать первый вопрос. "
        "Не используй одну и ту же фразу. Ответ должен быть ТОЛЬКО текстом реплики."
    )
    if not first_question:
        first_question = "Расскажите немного о себе и вашем опыте, релевантном для этой позиции."
    user_prompt = f"""
    Сгенерируй приветственную реплику.
    - Имя кандидата: {candidate_name}
    - Вакансия: {vacancy_title}
    - Первый вопрос: "{first_question}"
    Задача: Поздоровайся, представься ("Меня зовут Алекс") и плавно задай первый вопрос.
    Примеры:
    - "Добрый день, {candidate_name}! Я Алекс, ваш AI-интервьюер. Рад с вами пообщаться. Давайте начнем. {first_question}"
    - "{candidate_name}, приветствую! Меня зовут Алекс. Готовы? Отлично. {first_question}"
    Твой ответ:
    """
    response = _call_yandex_gpt(system_prompt, user_prompt, temperature=0.8)
